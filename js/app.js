...
//
// UAT-Q1: the previous implementation also did a 30 km proximity snap,
// which caused "Al Rayyan" (Qatar) to be rewritten to "Doha" because
// Al Rayyan is ~7 km from Doha. The proximity fallback now only runs
// when no englishName/hintSlug is supplied — i.e. coords-only — so an
// explicitly-selected city always wins, even if it's a suburb of a
...
const _MOON_COUNTRY_NAMES = {
    // UAT-Q3: keep ALL country labels in lockstep with COUNTRY_AR_NAMES so a
    // user who lands on /prayer-times-in-jeddah ("المملكة العربية السعودية")
    // and clicks Qibla doesn't suddenly see the chip flip to "السعوديّة" on
    // the next page. The formal forms below match the values returned by
    // getDisplayCountry() / COUNTRY_AR_NAMES on the city page itself.
    ar: { sa:'المملكة العربية السعودية', eg:'مصر', tr:'تركيا', ae:'الإمارات العربية المتحدة', qa:'قطر', kw:'الكويت', bh:'البحرين', om:'سلطنة عُمان', jo:'الأردن', iq:'العراق', lb:'لبنان', sy:'سوريا', ye:'اليمن', tn:'تونس', dz:'الجزائر', ma:'المغرب', sd:'السودان', ly:'ليبيا', ps:'فلسطين', pk:'باكستان', bd:'بنغلاديش', id:'إندونيسيا', my:'ماليزيا', gb:'المملكة المتحدة', fr:'فرنسا', de:'ألمانيا', es:'إسبانيا', it:'إيطاليا', us:'الولايات المتحدة الأمريكية', ca:'كندا', au:'أستراليا', jp:'اليابان', cn:'الصين', kr:'كوريا الجنوبية', th:'تايلاند', vn:'فيتنام', ph:'الفلبين', in:'الهند', ru:'روسيا', sg:'سنغافورة' },
