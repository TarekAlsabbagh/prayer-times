...
async function fetchLocalizedCityName(lat, lng) {
    ...
    try {
        ...
        // (UAT-2.8) قسم "عن المدينة" والاعتماد على Wikipedia API — أُزيلا.
...
async function loadCityData(lat, lng, city, country, countryCode = '', englishName = '', timezone = null) {
    ...
    // (UAT-2.8) قسم "عن المدينة" المعتمد على Wikipedia API أُزيل من صفحات المدينة.
...
// (UAT-2.8) loadCityAboutSection() — حُذف نهائيًّا مع قسم "عن المدينة" المعتمد على Wikipedia.
...
function updateHijriToday() {
    ...
    // ── 5. [REMOVED Round 9 + UAT-2.8] OTD historical events — moved off this page,
    //      and the Wikipedia-backed loaders (loadWikiOTD/loadHijriDayOTD/loadConverterOTD)
    //      were deleted in UAT-2.8 to drop the Wikipedia API dependency entirely.
...
// ========= [UAT-2.8] Wikipedia OTD code removed =========
//   loadHijriDayOTD / loadWikiOTD / loadConverterOTD / renderBio /
//   _nonIslamicKeywords / _wikiOTDLoaded / _converterOTDToken — all dead
//   code that depended on ar.wikipedia.org and was never invoked from any
//   live page. Kill-switched to drop the Wikipedia API surface entirely.
