# GR GeoNames Import Report — Europe-3

**Country**: Greece (اليونان)
**Wave**: `CURATED-GEODATA-EUROPE-3`
**Strategy**: E (popMin 100000 + alwaysInclude PPLC,PPLA + ar-quality gate)
**Generated**: 2026-05-16T08:16:12.984Z

## Pipeline

| Stage | Output |
| --- | --- |
| 1. IMPORT       | `db/places/candidates/gr-geonames-raw.json` |
| 2. NORMALIZE    | `db/places/candidates/gr-geonames-normalized.json` |
| 3. VALIDATE     | `db/places/candidates/gr-geonames-candidates.json` |
...
| Bucket | Count |
| --- | --- |
| Normalized candidates total       | 14237 |
| existing (matched, no action)     | 1 |
| **pending — high tier**           | **2** |
| pending — medium tier             | 0 |
| pending — low tier                | 135 |
| needs_review                      | 14094 |
| rejected                          | 0 |
| collisions in this wave           | 1054 |
| collisions against existing curated | 7 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| ⚠️ | corfu | شهر کورفو | Corfu | Corfu | gr | PPLA | 40047 |  | 39.6244 | 19.9202 | 376.95 | athens | mixed_script |  | 80 | always_include:PPLA |
| ⚠️ | mytilene | موتیلنه | Mytilene | Mytilene | gr | PPLA | 28322 |  | 39.1077 | 26.5553 | 275.84 | athens | mixed_script |  | 80 | always_include:PPLA |

## Collision-watch list for GR
...
2. For each **✅** row you want in curated:
   open `db/places/candidates/gr-geonames-candidates.json`,
   change `"status": "pending"` to `"status": "approved"`.
3. For each **⚠️** row:
   - if Arabic is bad: edit `candidate.names.ar` to a clean Arabic value;
   - if collision: change `candidate.slug` to the suggested `slug-gr` form;
...
Source: https://download.geonames.org/export/dump/GR.zip
