...
|---|---|---|
| `/moon/{country}` | **404 نظيف** من السيرفر (لا 200 فارغة، لا footer-only) | صفحة دولة SEO كاملة |
...
**القاعدة الآن:** السيرفر يُرجِع **404** (صفحة خطأ صغيرة ~2KB، ليست shell 200KB) لِكلّ مسار `/moon/{...}` متداخل. العميل (`_isMoonPath`) **يعرفها مسبقًا** كـ`page-moon` لِئلّا تتكرّر مشكلة footer-only عند تفعيلها لاحقًا — لكنّ السيرفر لا يُصدِر لها جسمًا حتّى تُبنى. **هذه الروابط لا تدخل sitemap قبل تفعيلها.**
...
E) `/moon/{...}` المستقبليّة = 404 نظيف (ليست 200/footer-only/shell).
F) sitemap: فيه `/moon`، ليس فيه `/moon-today`، ليس فيه `/moon/{country}`، لا إغراق أيّام.
