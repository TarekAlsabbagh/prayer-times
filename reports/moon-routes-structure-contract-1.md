...
**التاريخ:** 2026-06-17 (أُنشئ) · **آخر تحديث:** 2026-06-18 (`MOON-CITY-DAY-ROUTE-STRUCTURE-ADD-1` — فعّلت صفحة اليوم `/moon/{country}/{city}/{yyyy}/{mm}/{dd}` في قسم `page-moon-day` مستقلّ؛ والأعمق منها `/{dd}/…` و`today` وصور الشرطة dash يبقى 404 نظيف؛ روابط الأيّام في صفحة الشهر صارت تشير إلى اليوم المتداخل الجديد؛ صفحات اليوم index + canonical ذاتيّ لكن **غير مُضافة دفعةً إلى sitemap**). · سابقًا 2026-06-18 (`MOON-CITY-MONTH-ROUTE-STRUCTURE-ADD-1` — فعّلت صفحة الشهر `/moon/{country}/{city}/{yyyy}/{mm}` في قسم `page-moon-month` مستقلّ). · سابقًا 2026-06-18 (`MOON-CITY-YEAR-ROUTE-STRUCTURE-ADD-1` — صفحة السنة `/moon/{country}/{city}/{yyyy}`). · سابقًا 2026-06-18 (`MOON-CITY-HUB-ROUTE-STRUCTURE-ADD-1` — `/moon/{country}/{city}` + 301 من `/moon-in-{city}`).
...
| `/moon/{country}/{city}/{yyyy}/{mm}/{dd}` (+10 لغات، السنة 1900–2100، الشهر 01–12، اليوم `\d{2}` صالح حسب الشهر leap-aware) | **200** | ذاتيّ `…/{yyyy}/{mm}/{dd}` | **غير موجود** (index + canonical ذاتيّ، لكن **لا إضافة دفعة لِـ sitemap** — تُكتشَف عبر روابط أيّام صفحة الشهر) | **`page-moon-day`** (قسم مستقلّ) | **صفحة اليوم الجديدة** — breadcrumb 7 مستويات (رتبتا السنة والشهر روابط) + Hero (7 رقاقات: مدينة/تاريخ/يوم-أسبوع/tz/طور/إضاءة/عمر + مرساتان) + ملخّص اليوم (طور/إضاءة/عمر/أقرب مرحلة كبرى/tz) + جدول تفاصيل SSR (6 أعمدة) + رابط الشهر + يوم سابق/تالٍ (عبور الشهر والسنة) + 5 FAQ + روابط رجوع (شهر/سنة/مدينة) (MOON-CITY-DAY-ROUTE-STRUCTURE-ADD-1) |
...
E) `/moon/{country}` = **200** صفحة دولة؛ الهب المتداخل `/moon/{country}/{city}` = **200**؛ **صفحة السنة `…/{yyyy}` = 200 + `page-moon-year` نشط**؛ **صفحة الشهر `…/{yyyy}/{mm}` = 200 + `page-moon-month` نشط**؛ **صفحة اليوم `…/{yyyy}/{mm}/{dd}` = 200 + `page-moon-day` نشط + H1 واحد + 0 أقسام مُسرَّبة (leap-aware: 2024/02/29=200)**؛ الأعمق/غير الصالح `…/{today|yyyy/6|yyyy/00|yyyy/13|yyyy/mm/7|yyyy/mm/00|yyyy/mm/32|yyyy/02/30|yyyy/02/29-غير-كبيسة|yyyy/mm/dd/extra}` **وصور الشرطة dash `/{YYYY-MM}` + `/{YYYY-MM-DD}`** = **404 نظيف**؛ دولة/مدينة مجهولة = **404**؛ مدينة في دولة خاطئة = **301** للصحيح.
...
+ اختبار مخصّص `scripts/_smoke_moon_city_day_route_structure_add_1.mjs` (MOON-CITY-DAY-ROUTE-STRUCTURE-ADD-1): صفحة اليوم 200 + `page-moon-day` + H1 واحد + canonical ذاتيّ (4 مدن/لغات) + breadcrumb 7 مستويات DOM≡JSON-LD (AR+EN، رتبتا السنة/الشهر روابط) + 7 رقاقات + ملخّص يوم + جدول تفاصيل (6 أعمدة) + رابط الشهر + يوم سابق/تالٍ (عبور الشهر والسنة) + روابط رجوع (شهر/سنة/مدينة) + 5 FAQ + hreflang 10+x-default + لا Event + التحقّق (today/dash/`yyyy/6`/`yyyy/00`/`yyyy/13`/`yyyy/mm/7`/`yyyy/mm/00`/`yyyy/mm/32`/`yyyy/02/30`/أعمق = 404، leap-aware: 2024/02/29=200 و2026/02/29=404، mismatch=301) + sitemap (**صفحات اليوم غير مُضافة دفعةً**، أشهر موجودة) + روابط الشهر صارت للمتداخل + الروابط القديمة سليمة + Meeus ثابت.
