// CSP-CONNECT-SRC-CSI-GSTATIC-PING-1 — RED TESTS.
//
...
// must be fully green first. The base server (commit b7611a9) is booted ONCE from TP_BASE_ROOT and shared.
// Nothing here touches production.
//
// Usage: TP_BASE_ROOT=<base checkout> node scripts/_red_csp_connect_src_csi_gstatic_ping_1.mjs
...
const SMOKE = path.join(ROOT, 'scripts', '_smoke_csp_connect_src_csi_gstatic_ping_1.mjs');
const BASE_ROOT = process.env.TP_BASE_ROOT;
const BASE_PORT = 8797;
...
// the fixed connect-src segment — present exactly twice (enforcing first, Report-Only second)
const SEG = '_csAds + (_ADSENSE_ENABLED ? " https://csi.gstatic.com" : "") + _csTrafficQuality';
const SEG_WITHOUT = '_csAds + _csTrafficQuality';
const SEG_WILDCARD = '_csAds + (_ADSENSE_ENABLED ? " https://csi.gstatic.com https://*.gstatic.com" : "") + _csTrafficQuality';
const SEG_HTTPS = '_csAds + (_ADSENSE_ENABLED ? " https://csi.gstatic.com https:" : "") + _csTrafficQuality';
const SEG_STAR = '_csAds + (_ADSENSE_ENABLED ? " https://csi.gstatic.com *" : "") + _csTrafficQuality';
const IMG_F1 = 'https://*.google-analytics.com https://www.googletagmanager.com" : "") + (_ADSENSE_ENABLED ? " https://pagead2';
const IMG_F1_WITH_CSI = 'https://*.google-analytics.com https://www.googletagmanager.com https://csi.gstatic.com" : "") + (_ADSENSE_ENABLED ? " https://pagead2';
...
const CASES = [
    { id: 'R1',  name: 'host removed from the ENFORCING connect-src', expect: 'A', mut: s => replaceNth(s, SEG, SEG_WITHOUT, 0) },
    { id: 'R2',  name: 'host removed from the REPORT-ONLY connect-src', expect: 'B', mut: s => replaceNth(s, SEG, SEG_WITHOUT, 1) },
    { id: 'R3',  name: 'https://*.gstatic.com added to enforcing connect-src', expect: 'E', mut: s => replaceNth(s, SEG, SEG_WILDCARD, 0) },
    { id: 'R4',  name: 'bare https: added to Report-Only connect-src', expect: 'F', mut: s => replaceNth(s, SEG, SEG_HTTPS, 1) },
    { id: 'R5',  name: 'wildcard * added to enforcing connect-src', expect: 'F', mut: s => replaceNth(s, SEG, SEG_STAR, 0) },
    { id: 'R6',  name: 'another directive touched (enforcing img-src gains csi.gstatic.com)', expect: 'K', mut: s => replaceNth(s, IMG_F1, IMG_F1_WITH_CSI, 0) },
    { id: 'R7',  name: 'F1 reopened (googletagmanager removed from Report-Only img-src)', expect: 'K', mut: s => replaceNth(s, IMG_F1, IMG_F1_REMOVED, 1) },
    ...
    { id: 'R9',  name: 'Consent Mode default changed (wait_for_update 500 -> 501)', expect: 'M', mut: s => s.replace("'wait_for_update':500,", "'wait_for_update':501,") },
    { id: 'R10', name: 'AdSense page tag changed (crossorigin dropped)', expect: 'M', mut: s => s.replace(`_ADSENSE_CLIENT + '" crossorigin="anonymous"></script>'`, `_ADSENSE_CLIENT + '"></script>'`) },
    { id: 'R11', name: 'regional CMP suppression disabled for EEA visitors', expect: 'M', mut: s => s.split('if (_inRegulatedRegion) html = _stripCustomConsentBanner(html, urlPath);').join('if (false) html = _stripCustomConsentBanner(html, urlPath);') },
    { id: 'R12', name: 'SSR cache nonce bypass switched off', expect: 'W', mut: s => s.replace('const _SC_NONCE_BYPASS = true;', 'const _SC_NONCE_BYPASS = false;') },
];
