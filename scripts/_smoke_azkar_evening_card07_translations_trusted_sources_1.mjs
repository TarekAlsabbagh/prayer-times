...
for (const l of ALL9) ok((evRegion.match(new RegExp('translation_' + l + ':', 'g')) || []).length === 8, `evening region translation_${l}: EXACTLY 8 (001-004 Quran + 005-008 dua)`);
...
ok(E.slice(8).every(d => ALL9.every(l => d['translation_' + l] == null)), 'evening cards 009+ carry NO translation fields');
...
console.log('\n================ 8. Cache-busters bumped (azkar-data.js?v=40 + sw v538) ================');
ok(/js\/azkar-data\.js\?v=40\b/.test(htmlSrc), 'index.html loads js/azkar-data.js?v=40');
ok(!/js\/azkar-data\.js\?v=38\b/.test(htmlSrc), 'no stale ?v=38 azkar-data reference in index.html');
ok(/CACHE_VERSION\s*=\s*'v538'/.test(swSrc), "sw.js CACHE_VERSION = 'v538'");
