...
for (const l of ALL9) ok((evRegion.match(new RegExp('translation_' + l + ':', 'g')) || []).length === 9, `evening region translation_${l}: EXACTLY 9 (001-004 Quran + 005-009 dua)`);
...
ok(E.slice(9).every(d => ALL9.every(l => d['translation_' + l] == null)), 'evening cards 010+ carry NO translation fields');
...
console.log('\n================ 8. Cache-busters bumped (azkar-data.js?v=41 + sw v539; app.js?v=842 + style.css?v=500 STABLE) ================');
ok(/js\/azkar-data\.js\?v=41\b/.test(htmlSrc), 'index.html loads js/azkar-data.js?v=41');
ok(!/js\/azkar-data\.js\?v=40\b/.test(htmlSrc), 'no stale ?v=40 azkar-data reference in index.html');
ok(/CACHE_VERSION\s*=\s*'v539'/.test(swSrc), "sw.js CACHE_VERSION = 'v539'");
