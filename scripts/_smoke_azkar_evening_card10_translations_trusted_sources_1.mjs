...
for (const l of ALL9) ok((evRegion.match(new RegExp('translation_' + l + ':', 'g')) || []).length === 10, `evening region translation_${l}: EXACTLY 10 (001-004 Quran + 005-010 dua)`);
...
ok(E.slice(10).every(d => ALL9.every(l => d['translation_' + l] == null)), 'evening cards 011+ carry NO translation fields');
...
console.log('\n================ 8. Cache-busters bumped (azkar-data.js?v=42 + sw v540; app.js?v=842 + style.css?v=500 STABLE) ================');
ok(/js\/azkar-data\.js\?v=42\b/.test(htmlSrc), 'index.html loads js/azkar-data.js?v=42');
ok(!/js\/azkar-data\.js\?v=41\b/.test(htmlSrc), 'no stale ?v=41 azkar-data reference in index.html');
ok(/CACHE_VERSION\s*=\s*'v540'/.test(swSrc), "sw.js CACHE_VERSION = 'v540'");
