...
for (const l of ALL9) ok((evRegion.match(new RegExp('translation_' + l + ':', 'g')) || []).length === 14, `evening region translation_${l}: EXACTLY 14 (001-004 Quran + 005-014 dua)`);
...
console.log('\n================ 7. Evening 001-014 translated; 015+ untranslated; morning + prayer intact; morning-012 UNCHANGED ================');
for (let n = 1; n <= 14; n++) {
...
ok(E.slice(14).every(d => ALL9.every(l => d['translation_' + l] == null)), 'evening cards 015+ carry NO translation fields');
...
console.log('\n================ 9. Cache-busters bumped (azkar-data.js?v=46 + sw v544; app.js?v=842 + style.css?v=500 STABLE) ================');
ok(/js\/azkar-data\.js\?v=46\b/.test(htmlSrc), 'index.html loads js/azkar-data.js?v=46');
ok(!/js\/azkar-data\.js\?v=45\b/.test(htmlSrc), 'no stale ?v=43 azkar-data reference in index.html');
ok(/CACHE_VERSION\s*=\s*'v544'/.test(swSrc), "sw.js CACHE_VERSION = 'v544'");
