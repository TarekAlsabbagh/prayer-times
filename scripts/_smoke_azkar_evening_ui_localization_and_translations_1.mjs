...
// exactly 8 per non-ar lang in the EVENING region (001-004 Quran + 005-012 dua); rest-of-evening (009+) has none
...
for (const l of NONAR) ok((eveningRegion.match(new RegExp('translation_' + l + ':', 'g')) || []).length === 12, `translation_${l}: appears EXACTLY 12 times in the evening region`);
ok(E.slice(12).every(d => NONAR.every(l => d['translation_' + l] == null)), 'evening cards 013+ carry NO translation fields');
...
console.log('\n================ 9. Cache-busters ================');
ok(/js\/azkar-data\.js\?v=44/.test(htmlSrc), 'index.html azkar-data.js?v=44 (bumped by the Card 09 ticket)');
ok(/js\/app\.js\?v=842/.test(htmlSrc), 'index.html app.js?v=842');
ok(/CACHE_VERSION = 'v542'/.test(swSrc), "sw.js CACHE_VERSION 'v542' (bumped by the Card 09 ticket)");
