...
// exactly 8 per non-ar lang in the EVENING region (001-004 Quran + 005-019 dua); rest-of-evening (009+) has none
...
for (const l of NONAR) ok((eveningRegion.match(new RegExp('translation_' + l + ':', 'g')) || []).length === 19, `translation_${l}: appears EXACTLY 19 times in the evening region`);
ok(E.slice(19).every(d => NONAR.every(l => d['translation_' + l] == null)), 'evening cards 020+ carry NO translation fields');
...
console.log('\n================ 9. Cache-busters ================');
ok(/js\/azkar-data\.js\?v=51/.test(htmlSrc), 'index.html azkar-data.js?v=51 (bumped by the Card 09 ticket)');
ok(/js\/app\.js\?v=842/.test(htmlSrc), 'index.html app.js?v=842');
ok(/CACHE_VERSION = 'v549'/.test(swSrc), "sw.js CACHE_VERSION 'v549' (bumped by the Card 09 ticket)");
