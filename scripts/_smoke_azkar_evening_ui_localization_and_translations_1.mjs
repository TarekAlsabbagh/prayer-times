...
// exactly 7 per non-ar lang in the EVENING region (001-004 Quran + 005-007 dua); rest-of-evening (008+) has none
...
for (const l of NONAR) ok((eveningRegion.match(new RegExp('translation_' + l + ':', 'g')) || []).length === 7, `translation_${l}: appears EXACTLY 7 times in the evening region`);
ok(E.slice(7).every(d => NONAR.every(l => d['translation_' + l] == null)), 'evening cards 008+ carry NO translation fields');
...
console.log('\n================ 9. Cache-busters ================');
ok(/js\/azkar-data\.js\?v=39/.test(htmlSrc), 'index.html azkar-data.js?v=39 (bumped by the Card 09 ticket)');
ok(/js\/app\.js\?v=838/.test(htmlSrc), 'index.html app.js?v=838');
ok(/CACHE_VERSION = 'v536'/.test(swSrc), "sw.js CACHE_VERSION 'v536' (bumped by the Card 09 ticket)");
