...
// exactly 8 per non-ar lang in the EVENING region (001-004 Quran + 005-023 dua); rest-of-evening (009+) has none
...
for (const l of NONAR) ok((eveningRegion.match(new RegExp('translation_' + l + ':', 'g')) || []).length === 23, `translation_${l}: appears EXACTLY 23 times in the evening region`);
ok(E.slice(23).every(d => NONAR.every(l => d['translation_' + l] == null)), 'evening cards 024+ carry NO translation fields');
