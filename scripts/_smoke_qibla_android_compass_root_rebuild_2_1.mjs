...
//     all 10 langs; help/fallback + recalibrate present; NO offset; NO Compass Lab; DOM+CSS hooks.
import fs from 'fs';
...
ok(/function recalibrateCompass\(\)/.test(appSrc) && /window\.recalibrateCompass = recalibrateCompass/.test(appSrc), 'recalibrateCompass() exposed globally');
...
for (const ln of ['ar','en','fr','tr','ur','de','id','es','bn','ms']) {
  ok(new RegExp('\\b' + ln + ':\\s*\\{\\s*low:').test(appSrc), 'L10N has lang ' + ln + ' (low/hint/recal/unavail)');
}
...
ok(!/AbsoluteOrientationSensor/.test(appCode), 'NO AbsoluteOrientationSensor in code (deferred per decision #2)');
...
ok(/js\/app\.js\?v=820/.test(htmlSrc), 'index.html app.js?v=820');
ok(/css\/style\.css\?v=491/.test(htmlSrc), 'index.html css/style.css?v=491');
ok(/CACHE_VERSION = 'v489'/.test(fs.readFileSync(path.join(ROOT,'sw.js'),'utf8')), 'sw.js CACHE_VERSION v489');
