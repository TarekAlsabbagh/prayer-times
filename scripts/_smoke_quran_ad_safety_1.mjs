 ...
 * bump the cache-buster to v24, and keep the Tanzil attribution intact. Pure-Node static analysis, no server.
 */
import fs from 'fs';
...
// ---- cache-buster bumped to v24 exactly (no v23, no v25) ----
const v24 = (server.match(/quran\.css\?v=24/g) || []).length;
ok(v24 === 2, 'quran.css?v=24 injected on both Quran routes (got ' + v24 + ')');
ok(!/quran\.css\?v=23/.test(server), 'no stale quran.css?v=23 remains');
ok(!/quran\.css\?v=25/.test(server), 'no premature quran.css?v=25');
...
if (fail === 0) console.log('  QURAN PAGES CARRY ZERO REAL AD CODE, A READING-SURFACE AD BACKSTOP, DOCUMENTED AUTO-ADS EXCLUSION, v24');
