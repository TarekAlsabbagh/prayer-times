// Smoke — QURAN ayah jump: numeric input bounded by THIS surah's own ayah count, with JS smooth-scroll +
// highlight AND a no-JS server Form/Redirect fallback (GET /quran/{slug}?ayah=N -> 302 /quran/{slug}#ayah-N —
// the SAME slug plus a fragment, never a different path). No innerHTML text writes.
...
ok(/<form class="quran-jump quran-ayah-jump" method="get" action="\$\{_quranPathFor\(surah\.surah\)\}" data-quran-ayah-jump>/.test(b), 'ayah-jump is a GET form posting to the CURRENT surah route, built by _quranPathFor (no-JS submits)');
...
// The Location is built from `_qs.path` — the SAME slug that was requested — so this can only ever be a
// fragment jump within one page. Concatenating a path here would silently turn it into a URL redirect.
ok(/res\.writeHead\(302, \{ Location: _qs\.path \+ '#ayah-' \+ a \}\)/.test(src), 'server 302s a valid ayah to the #ayah-N fragment OF THE REQUESTED SLUG (never another path)');
