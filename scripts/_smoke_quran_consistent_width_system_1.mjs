...
// The editorial block split into _quranEditorialHtml (surah-21-gated) + _quranToolsHtml (all 114), and the
// source/FAQ builders now take the display name — same boxes, same one container, new signatures.
['moon-breadcrumb', 'quran-hero', 'quran-toolbar', 'quran-progress', '${pagesHtml}', 'quran-surah-end', '${_quranEditorialHtml(surah, sName)}', '${_quranToolsHtml(surah, sName)}', '${_quranSourceHtml(surah, manifest, sName)}', '${_quranFaqHtml(surah, sName)}', "_quranServiceLinksHtml('quran-services-full')"].forEach(k =>
