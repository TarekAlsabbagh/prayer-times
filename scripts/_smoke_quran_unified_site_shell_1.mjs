...
ok(/\(process\.env\.QURAN_PROTOTYPE_ENABLED === '1' && !!\(_quranSurahRoute\(urlPath\) \|\| \{\}\)\.canonical\)/.test(src), 'route added to _isIndexHtmlRoute (flag-gated) — served as index.html shell');
...
ok(/if \(\/\^\\\/quran\\\/surah\\\/\(\?:\[1-9\]\|\[1-9\]\[0-9\]\|10\[0-9\]\|11\[0-4\]\)\$\/\.test\(window\.location\.pathname\)/.test(app), 'app.js initApp activates #page-quran-surah');
ok(/\} else if \(\/\^\\\/quran\\\/surah\\\/\(\?:\[1-9\]\|\[1-9\]\[0-9\]\|10\[0-9\]\|11\[0-4\]\)\$\/\.test\(_path\)\) \{[\s\S]*_expectedId = 'page-quran-surah'/.test(app), 'app.js pageshow self-heal keeps #page-quran-surah (no flash-to-home)');
