...
// ─── Group 3: js/app.js — _syncCityNameInDom has reverse guard ──────────
console.log('');
console.log('── Group 3: _syncCityNameInDom has REVERSE absence-lang guard ──');

// The fix adds: `if (_isAbsenceLang && _hasLatin(ssrName) && _hasArOrBnEarly) return;`
// Verify both: original guard for non-Latin ssrName + Latin goodName,
// AND new reverse guard for Latin ssrName + non-Latin goodName.
...
ok('_syncCityNameInDom NEW reverse guard (Latin ssrName, non-Latin goodName) present',
...
ok('index.html references js/app.js?v=665',
   /js\/app\.js\?v=665/.test(INDEX_HTML));
ok('index.html does NOT still reference js/app.js?v=664 (old)',
