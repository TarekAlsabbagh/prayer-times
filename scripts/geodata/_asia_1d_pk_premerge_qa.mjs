// scripts/geodata/_asia_1d_pk_premerge_qa.mjs
// ─────────────────────────────────────────────────────────────────────────
// ASIA-1D-PK-PREMERGE-QA-1 — pre-merge quality assurance for passes-gate
// entries from ASIA-1D-PK (Pakistan only).
//
// Inputs:
//   db/places/candidates/pk-geonames-candidates.json (post-Stage-3.4 + 3.5)
//   db/places/curated-places.json
//
// Output:
//   reports/geodata-asia-1d-pk-premerge-qa.md
//
// Does NOT modify any input. Same 8 checks as IR + adapted PK
// compound hints and watch-list.
// ─────────────────────────────────────────────────────────────────────────
...
// User-flagged watch-list — 22 Pakistani cities to scrutinize
// (10 existing curated + 12 major non-curated cities expected in wave)
const WATCHLIST = [
    // Existing curated (should match status='existing' or trivially)
    'karachi', 'lahore', 'islamabad', 'rawalpindi', 'peshawar',
    'multan', 'faisalabad', 'quetta', 'hyderabad', 'hyderabad-pk', 'sialkot',
    // Major non-curated cities expected to enter via this wave
    'bahawalpur', 'gujranwala', 'sargodha', 'sukkur', 'larkana',
    'mardan', 'sheikhupura', 'sahiwal', 'mingora', 'okara',
    'gujrat', 'kasur', 'rahim-yar-khan', 'rahimyar-khan',
    'wah', 'wah-cantonment', 'jhang', 'chiniot', 'dera-ghazi-khan',
    'dera-ismail-khan', 'gilgit', 'muzaffarabad'
];

// PK-specific compound-name hints
//   "abad" = آباد — extremely common (Islamabad, Faisalabad, Hyderabad)
//   "pur"  = پور  — common in Punjab (Bahawalpur)
//   "kot"  = كوت  — common (Sialkot, Pasrur)
//   "khan" = خان
//   "shahr" = شهر — same as IR
const COMPOUND_HINTS = {
    'abad':  { en: 'abad',   ar_expected: 'آباد', desc: '"X-abad"' },
    'pur':   { en: 'pur',    ar_expected: 'بور',  desc: '"X-pur"' },
    'kot':   { en: 'kot',    ar_expected: 'كوت',  desc: '"X-kot"' },
...
function main() {
    ...
    // === Build report ===
    const lines = [];
    lines.push('# ASIA-1D-PK Pre-Merge QA Report');
    lines.push('');
    lines.push('**Wave**: `CURATED-GEODATA-ASIA-1D-PK`');
    lines.push('**Country**: Pakistan (باكستان)');
    ...
    lines.push('');
    lines.push('User-flagged Pakistani cities (curated + expected major new entries):');
    ...
    if (!watchHits.length) {
        ...
        for (const w of watchHits) {
            ...
            if (w.matchedExisting.length && !w.passingGateInWave.length && !w.blockedInWave.length) {
                ...
                rec = '⏭️ deferred to ASIA-1D-PK-MCF';
            } else {
                rec = '—';
            }
    ...
    if (!majorBlockedAutoReco.length) {
        ...
        lines.push('Major (pop ≥ 200k OR PPLC/PPLA) high-tier entries CURRENTLY BLOCKED. Candidates for a future `ASIA-1D-PK-MCF` mini-phase.');
        lines.push('');
    ...
    const outPath = path.join(BASE_PATHS.reportDir, 'geodata-asia-1d-pk-premerge-qa.md');
    fs.writeFileSync(outPath, lines.join('\n'));
    console.log('[qa] wrote ' + outPath);

    console.log('');
    console.log('═══ ASIA-1D-PK Pre-Merge QA ═══');
