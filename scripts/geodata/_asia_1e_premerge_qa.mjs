...
// ─────────────────────────────────────────────────────────────────────────
// ASIA-1E-PREMERGE-QA-1 — pre-merge quality assurance for passes-gate
// entries from ASIA-1E (NP + LK + MV + BT + BN + MM + KH + LA + TL).
// Generates a comprehensive QA report at:
//   reports/geodata-asia-1e-premerge-qa.md
//
...
// Common compound-name tokens for ASIA-1E (small Asian misc patterns)
...
function main() {
    ...
    const lines = [];
    lines.push('# ASIA-1E Pre-Merge QA Report');
    lines.push('');
    lines.push('**Wave**: `CURATED-GEODATA-ASIA-1E`');
    ...
    if (!majorBlockedAutoReco.length) {
        ...
        lines.push('These are candidates for a future `ASIA-1E-BLOCKED-MAJOR-CITIES-FIX-1` mini-phase.');
    ...
    const outPath = path.join(BASE_PATHS.reportDir, 'geodata-asia-1e-premerge-qa.md');
    fs.writeFileSync(outPath, lines.join('\n'));
    console.log('[qa] wrote ' + outPath);

    // Console summary
    console.log('');
    console.log('═══ ASIA-1E Pre-Merge QA ═══');
