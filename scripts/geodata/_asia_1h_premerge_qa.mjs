// scripts/geodata/_asia_1e_premerge_qa.mjs
// ─────────────────────────────────────────────────────────────────────────
// ASIA-1H-PREMERGE-QA-1 — pre-merge quality assurance for passes-gate
// entries from ASIA-1H (UZ + KZ + TJ + KG + TM + MN — Central Asia + Mongolia).
// Generates a comprehensive QA report at:
//   reports/geodata-asia-1h-premerge-qa.md
//
...
const PERSIAN_URDU = /[پچژگٹڈڑښګڵݫݬیکہےۀڤڥڨۆۇۈېە]/;  // includes Uyghur chars
const LATIN_IN_AR = /[A-Za-z]/;

// User-flagged watchlist (Central Asia + Mongolia)
const WATCHLIST = [
...
// Common compound-name tokens for ASIA-1H (Central Asia + Mongolia patterns)
const COMPOUND_HINTS = {
    ...
    // Burmese / Khmer / Lao / Sinhala / Dhivehi don't carry obvious English suffixes
    // but a few exist
    'town':         { en: 'town',         ar_expected: 'بلدة',     desc: '"X Town"' },
    'point':        { en: 'point',        ar_expected: 'بوينت',    desc: '"X Point"' },
    'district':     { en: 'district',     ar_expected: 'منطقة',    desc: '"X District"' }
};
...
function main() {
    ...
    const lines = [];
    lines.push('# ASIA-1H Pre-Merge QA Report');
    lines.push('');
    lines.push('**Wave**: `CURATED-GEODATA-ASIA-1H`');
    ...
    if (!majorBlockedAutoReco.length) {
        ...
        lines.push('These are candidates for a future `ASIA-1H-BLOCKED-MAJOR-CITIES-FIX-1` mini-phase.');
    ...
    const outPath = path.join(BASE_PATHS.reportDir, 'geodata-asia-1h-premerge-qa.md');
    fs.writeFileSync(outPath, lines.join('\n'));
    console.log('[qa] wrote ' + outPath);

    // Console summary
    console.log('');
    console.log('═══ ASIA-1H Pre-Merge QA ═══');
