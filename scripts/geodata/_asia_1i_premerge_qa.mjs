...
// ─────────────────────────────────────────────────────────────────────────
// ASIA-1I-PREMERGE-QA-1 — pre-merge quality assurance for passes-gate
// entries from ASIA-1I (AZ + GE + AM — Caucasus).
// Generates a comprehensive QA report at:
//   reports/geodata-asia-1i-premerge-qa.md
//
...
// User-flagged watchlist for collision review (Caucasus capitals + secondaries)
const WATCHLIST = [
...
// Common compound-name tokens for ASIA-1I (Caucasus patterns)
...
function main() {
    ...
    const lines = [];
    lines.push('# ASIA-1I Pre-Merge QA Report');
    lines.push('');
    lines.push('**Wave**: `CURATED-GEODATA-ASIA-1I`');
    ...
    if (!majorBlockedAutoReco.length) {
        ...
        lines.push('These are candidates for a future `ASIA-1I-BLOCKED-MAJOR-CITIES-FIX-1` mini-phase.');
    ...
    const outPath = path.join(BASE_PATHS.reportDir, 'geodata-asia-1i-premerge-qa.md');
    fs.writeFileSync(outPath, lines.join('\n'));
    console.log('[qa] wrote ' + outPath);

    // Console summary
    console.log('');
    console.log('═══ ASIA-1I Pre-Merge QA ═══');
