...
// ─────────────────────────────────────────────────────────────────────────
// CURATED-GEODATA-AMERICAS-1A — Strategy E report generator (ES + PT)
//
// Inputs:
//   - db/places/candidates/{cc}-geonames-candidates.json   (Stage 3 output,
//                                                          enriched by Stage 3.5)
//   - db/places/candidates/americas-1a-arabic-quality.json   (Stage 3.5 summary)
//   - db/places/curated-places.json
//   - db/places/candidates/{cc}-geonames-raw.json          (for originalName)
//
// Outputs:
//   - reports/geodata-americas-1a-summary.md
//   - reports/geodata-americas-1a-arabic-quality-report.md
//   - reports/{cc}-geodata-import-report.md   (REWRITTEN per Americas-1A
//                                              template with 14 fields)
//
// Does NOT touch curated_places.json. Does NOT apply anything.
// Cloned + parameterized from `_strategy_e_europe_3_reports.mjs` so the
...
const WAVE_LABEL = 'CURATED-GEODATA-AMERICAS-1A';
const WAVE_SLUG  = 'americas-1a';
const STRATEGY   = 'E';
const CCS        = ['us','ca','mx'];

// Cities the user flagged for explicit collision check (Strategy E §6)
const WATCH_LIST = ['birmingham','manchester','cambridge','dublin','athens','saint-petersburg','toledo','rochester','salem','victoria','cordoba','merida','leon','granada','santiago','washington','new-york','los-angeles','chicago','montreal','toronto','vancouver','mexico-city','guadalajara','monterrey','newcastle','peterborough','york'];
...
function renderCountryReport(cc, candidates, rawIndex, config) {
    ...
    lines.push('# ' + upper + ' GeoNames Import Report — Americas-1A');
    lines.push('');
...
function renderArQualityReport(perCountryResults) {
    const lines = [];
    lines.push('# Americas-1A — Arabic-Name Quality Report');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('');
    lines.push('Same as Europe-1A: North America has Arabic-name candidates');
    ...
    lines.push('| Within Americas-1A wave (ES↔PT same slug) | ' + waveColl + ' |');
...
function renderSummary(perCountryResults) {
    const lines = [];
    lines.push('# Americas-1A — Wave Summary');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('| --- | --- |');
    lines.push('| Wave summary (this file) | `reports/geodata-americas-1a-summary.md` |');
    lines.push('| Arabic-quality detail   | `reports/geodata-americas-1a-arabic-quality-report.md` |');
    for (const cc of CCS) {
