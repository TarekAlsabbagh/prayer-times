// scripts/geodata/_strategy_e_asia_1c_reports.mjs
// ─────────────────────────────────────────────────────────────────────────
// CURATED-GEODATA-ASIA-1C — Strategy E report generator
// (JP + KR + HK + TW + MO — East Asia CJK + SAR)
//
// Inputs:
//   - db/places/candidates/{cc}-geonames-candidates.json   (Stage 3 + 3.5)
//   - db/places/candidates/asia-1c-arabic-quality.json     (Stage 3.5 summary)
//   - db/places/curated-places.json
//   - db/places/candidates/{cc}-geonames-raw.json          (for originalName)
//
// Outputs (7 files):
//   - reports/geodata-asia-1c-summary.md
//   - reports/geodata-asia-1c-arabic-quality-report.md
//   - reports/{cc}-geodata-import-report.md (jp, kr, hk, tw, mo)
//
...
const WAVE_LABEL = 'CURATED-GEODATA-ASIA-1C';
const WAVE_SLUG  = 'asia-1c';
const STRATEGY   = 'E';
const CCS        = ['jp','kr','hk','tw','mo'];

// User-flagged collision watch list (kickoff message, 2026-05-17):
//   Major already-curated CJK cities + obvious collision risks.
const WATCH_LIST = ['tokyo','osaka','kyoto','yokohama','nagoya','sapporo','sendai','nara','okinawa',
...
function renderCountryReport(cc, candidates, rawIndex, config) {
    ...
    lines.push('# ' + upper + ' GeoNames Import Report — Asia-1C');
    lines.push('');
...
function renderArQualityReport(perCountryResults) {
    const lines = [];
    lines.push('# Asia-1C — Arabic-Name Quality Report');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('| Within Asia-1C wave (JP/KR/HK/TW/MO same slug) | ' + waveColl + ' |');
...
function renderSummary(perCountryResults) {
    const lines = [];
    lines.push('# Asia-1C — Wave Summary');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('## Collision-watch list (user-specified, Asia-1C kickoff)');
    lines.push('');
    ...
    lines.push('| --- | --- |');
    lines.push('| Wave summary (this file) | `reports/geodata-asia-1c-summary.md` |');
    lines.push('| Arabic-quality detail   | `reports/geodata-asia-1c-arabic-quality-report.md` |');
    for (const cc of CCS) {
