// scripts/geodata/_strategy_e_asia_1e_reports.mjs
// ─────────────────────────────────────────────────────────────────────────
// CURATED-GEODATA-ASIA-1E — Strategy E report generator
// (NP + LK + MV + BT + BN + MM + KH + LA + TL — small Asian misc)
//
// Inputs:
//   - db/places/candidates/{cc}-geonames-candidates.json   (Stage 3 + 3.5)
//   - db/places/candidates/asia-1e-arabic-quality.json     (Stage 3.5 summary)
//   - db/places/curated-places.json
//   - db/places/candidates/{cc}-geonames-raw.json          (for originalName)
//
// Outputs (11 files):
//   - reports/geodata-asia-1e-summary.md
//   - reports/geodata-asia-1e-arabic-quality-report.md
//   - reports/{cc}-geodata-import-report.md (np, lk, mv, bt, bn, mm, kh, la, tl)
//
...
const WAVE_LABEL = 'CURATED-GEODATA-ASIA-1E';
const WAVE_SLUG  = 'asia-1e';
const STRATEGY   = 'E';
const CCS        = ['np','lk','mv','bt','bn','mm','kh','la','tl'];

// Watch list: notable capitals + likely collision candidates
const WATCH_LIST = [
    'kathmandu','pokhara','janakpur','butwal',
    'colombo','kandy','jaffna','galle','trincomalee',
    'male','malé','hithadhoo',
    'thimphu','paro','punakha',
    'bandar-seri-begawan','kuala-belait','tutong',
    'yangon','mandalay','naypyidaw','nay-pyi-taw','mawlamyine','pathein','bago','sittwe',
    'phnom-penh','siem-reap','battambang','sihanoukville','takeo',
    'vientiane','savannakhet','luang-prabang','pakse','thakhek',
...
function renderCountryReport(cc, candidates, rawIndex, config) {
    ...
    lines.push('# ' + upper + ' GeoNames Import Report — Asia-1E');
    lines.push('');
    ...
    lines.push('| collisions in this wave (high)    | ' + counts.collisionsInWave + ' |');
    lines.push('| collisions against existing curated (high) | ' + counts.collisionsAgainstCurated + ' |');
    lines.push('');
    ...
    lines.push('`feature_code`, `population`, `admin region`, `lat`, `lng`, `timezone`,');
    ...
    if (!highTierEntries.length) {
        ...
        lines.push('| ✓ | slug | name.ar | name.en | originalName | cc | fc | pop | region | lat | lng | tz | nearestKm | nearest | arQuality | collision | suggestedRename | priority | reason |');
        lines.push('| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |');
    ...
    lines.push('');
    lines.push('Cities the user pre-flagged (kickoff 2026-05-16): `' + WATCH_LIST.join('`, `') + '`.');
    lines.push('');
...
function renderArQualityReport(perCountryResults) {
    const lines = [];
    lines.push('# Asia-1E — Arabic-Name Quality Report');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    lines.push('**Strategy**: ' + STRATEGY + ' — Strategy E + Stage 3.5 ar-quality gate');
    lines.push('**Generated**: ' + new Date().toISOString());
    lines.push('');
    lines.push('## What this report tells you');
    lines.push('');
    lines.push('Same as Americas-1A/1B: Asian Wikipedia ar: tags have moderate');
    lines.push('Persian/Urdu/Pashto contamination in GeoNames; Strategy E gate');
    lines.push('separates clean Arabic from contaminated. JP/KR/HK/TW/MO CJK East Asia');
    lines.push('have strong Arabic Wikipedia coverage for major cities (passes-gate ~70-80%');
    lines.push('expected — Japan in particular is well-covered; Macao single PPLC may block).');
    lines.push('');
    ...
    lines.push('');
    lines.push('## Aggregate summary (high-tier only)');
    lines.push('');
    lines.push('| Country | high-tier | wikidata | arabic_only | mixed_script | mixed_latin | mixed_unknown | empty | passes-gate | blocked |');
    lines.push('| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |');
    let totalHigh = 0, totalPasses = 0, totalBlocked = 0;
    let totalWiki = 0, totalArOnly = 0, totalMixedScript = 0, totalMixedLatin = 0, totalMixedUnk = 0, totalEmpty = 0;
    for (const cc of CCS) {
        ...
        const mu = r.counts.arHigh.mixed_unknown || 0;
        ...
        totalWiki += wd; totalArOnly += ao; totalMixedScript += ms; totalMixedLatin += ml; totalMixedUnk += mu; totalEmpty += em;
        lines.push('| ' + cc.toUpperCase() + ' | ' + high + ' | ' + wd + ' | ' + ao + ' | ' + ms + ' | ' + ml + ' | ' + mu + ' | ' + em + ' | **' + pg + '** | **' + bg + '** |');
    }
    lines.push('| **TOTAL** | **' + totalHigh + '** | **' + totalWiki + '** | **' + totalArOnly + '** | **' + totalMixedScript + '** | **' + totalMixedLatin + '** | **' + totalMixedUnk + '** | **' + totalEmpty + '** | **' + totalPasses + '** | **' + totalBlocked + '** |');
    lines.push('');

    // Collision summary
    lines.push('## Collision summary (high-tier only)');
    lines.push('');
    ...
    lines.push('| Within Asia-1E wave (NP/LK/MV/BT/BN/MM/KH/LA/TL same slug) | ' + waveColl + ' |');
    ...
    if (waveCollDetails.length) {
        ...
        lines.push('');
        lines.push('| cc | slug | suggestedRename | name.ar | name.en | pop |');
        lines.push('| --- | --- | --- | --- | --- | --- |');
        for (const { cc, e } of waveCollDetails) {
            lines.push('| ' + cc + ' | ' + md(e.slug) + ' | ' + md(e.suggestedSlugIfCollision || '') + ' | ' + md(e.candidate.names.ar) + ' | ' + md(e.candidate.names.en) + ' | ' + md(e.candidate.population) + ' |');
        }
        lines.push('');
    }
    if (curatedCollDetails.length) {
        ...
        lines.push('');
        lines.push('| cc | slug | existingCc | suggestedRename | name.ar | name.en | pop |');
        lines.push('| --- | --- | --- | --- | --- | --- | --- |');
        for (const { cc, e } of curatedCollDetails) {
            lines.push('| ' + cc + ' | ' + md(e.slug) + ' | ' + md(e.collisionAgainstCurated && e.collisionAgainstCurated.existingCc) + ' | ' + md(e.suggestedSlugIfCollision || '') + ' | ' + md(e.candidate.names.ar) + ' | ' + md(e.candidate.names.en) + ' | ' + md(e.candidate.population) + ' |');
        }
        lines.push('');
    }
...
function renderSummary(perCountryResults) {
    const lines = [];
    lines.push('# Asia-1E — Wave Summary');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    lines.push('**Strategy**: ' + STRATEGY + ' (popMin + alwaysInclude + ar-quality gate)');
    lines.push('**Countries**: ' + CCS.map(c => c.toUpperCase()).join(', '));
    lines.push('**Generated**: ' + new Date().toISOString());
    lines.push('');
    lines.push('## Filter thresholds (same as Americas-1A)');
    lines.push('');
    ...
    lines.push('## Collision-watch list (user-specified, Asia-1E kickoff)');
    lines.push('');
    lines.push('User pre-flagged these slugs for explicit collision check:');
    lines.push('');
    lines.push('`' + WATCH_LIST.join('`, `') + '`');
    lines.push('');
    lines.push('### Status of watch-list slugs (existing curated + current wave high-tier)');
    lines.push('');
    ...
    lines.push('| watch-slug | existing in curated? | wave candidate(s) high-tier | resolution recommendation |');
    lines.push('| --- | --- | --- | --- |');
    for (const w of WATCH_LIST) {
        ...
        // Wave candidates — only show high-tier
        const waveCands = [];
        for (const cc of CCS) {
            const r = perCountryResults[cc];
            if (!r) continue;
            for (const { e } of r.highTierEntries) {
                if (e.slug === w || e.slug.startsWith(w + '-')) {
                    waveCands.push(cc + '/' + e.slug + ' pop=' + (e.candidate.population || 0) + (e.pendingAfterArGate ? ' ✅' : ' ⚠️'));
                }
            }
        }
        const waveTxt = waveCands.length ? waveCands.join(' + ') : '_(not in high-tier)_';
        // Resolution recommendation — flag any decision points needing user input
        let resolution;
        if (exact && !waveCands.length) {
            resolution = 'bare slug already owned by ' + exact.countryCode + ' — wave is unaffected';
        } else if (exact && waveCands.length) {
            resolution = '⚠️ bare slug owned by ' + exact.countryCode + ' — new wave candidate must take `slug-<cc>` suffix';
        } else if (ccSuffixed.length && !waveCands.length) {
            resolution = 'pre-reserved with `-cc` suffix — bare slug **free** for future';
        } else if (ccSuffixed.length && waveCands.length === 1) {
            resolution = 'only one wave candidate — may claim bare slug **OR** take `-' + waveCands[0].split('/')[0] + '` suffix (user decision)';
        } else if (!exact && !ccSuffixed.length && waveCands.length === 1) {
            resolution = 'wave candidate claims bare slug (no existing reservation)';
        } else if (!exact && !ccSuffixed.length && waveCands.length >= 2) {
            resolution = '⚠️ **multiple wave candidates** — user must decide which gets bare slug; others take `-cc` suffix';
        } else if (!waveCands.length) {
            resolution = 'not in this wave';
        } else {
            resolution = 'multiple high-tier candidates AND existing curated — user decision needed';
        }
        lines.push('| `' + w + '` | ' + existing + ' | ' + waveTxt + ' | ' + resolution + ' |');
    }
    lines.push('');

    lines.push('## Strategy E decision: passes-gate vs blocked');
    lines.push('');
    lines.push('* **Passes ar-gate (' + totalPasses + ')** — ready for `status: approved` flip after collision resolution.');
    lines.push('* **Blocked by ar-gate (' + totalBlocked + ')** — manual fix BEFORE approval (Persian/Urdu/Latin/empty Arabic OR collision).');
    lines.push('');
    ...
    lines.push('| --- | --- |');
    lines.push('| Wave summary (this file) | `reports/geodata-asia-1e-summary.md` |');
    lines.push('| Arabic-quality detail   | `reports/geodata-asia-1e-arabic-quality-report.md` |');
    for (const cc of CCS) {
    ...
    lines.push('3. Reply to the assistant: `approve all` / `approve per-country` /');
    lines.push('   `fix Arabic` / `exclude slugs` / `rename slugs` / specific decisions');
    lines.push('   for watch-list ambiguities.');
