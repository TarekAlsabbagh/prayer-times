...
// ─────────────────────────────────────────────────────────────────────────
// CURATED-GEODATA-ASIA-1H — Strategy E report generator
// (UZ + KZ + TJ + KG + TM + MN — Central Asia + Mongolia)
//
// Inputs:
//   - db/places/candidates/{cc}-geonames-candidates.json   (Stage 3 + 3.5)
//   - db/places/candidates/asia-1h-arabic-quality.json     (Stage 3.5 summary)
//   - db/places/curated-places.json
//   - db/places/candidates/{cc}-geonames-raw.json          (for originalName)
//
// Outputs (8 files):
//   - reports/geodata-asia-1h-summary.md
//   - reports/geodata-asia-1h-arabic-quality-report.md
//   - reports/{cc}-geodata-import-report.md (uz, kz, tj, kg, tm, mn)
//
...
const WAVE_LABEL = 'CURATED-GEODATA-ASIA-1H';
const WAVE_SLUG  = 'asia-1h';
const STRATEGY   = 'E';
const CCS        = ['uz','kz','tj','kg','tm','mn'];

// Watch list: user-flagged collision candidates (Central Asia + Mongolia)
const WATCH_LIST = [
    'tashkent','samarkand','bukhara','namangan','andijan','nukus','fergana','qarshi',
    'astana','almaty','shymkent','karaganda','aktobe','atyrau','taraz','pavlodar',
    'oskemen','semey','kyzylorda','kostanay','turkestan',
    'dushanbe','khujand','bokhtar','kulob','khorugh',
    'bishkek','osh','jalal-abad','karakol','tokmok',
    'ashgabat','turkmenabat','dasoguz','mary','balkanabat',
    'ulaanbaatar','erdenet','darkhan','choibalsan','olgii','khovd'
];
...
function renderCountryReport(cc, candidates, rawIndex, config) {
    ...
    lines.push('# ' + upper + ' GeoNames Import Report — Asia-1H');
    lines.push('');
...
function renderArQualityReport(perCountryResults) {
    const lines = [];
    lines.push('# Asia-1H — Arabic-Name Quality Report');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('| Within Asia-1H wave (NP/LK/MV/BT/BN/MM/KH/LA/TL same slug) | ' + waveColl + ' |');
...
function renderSummary(perCountryResults) {
    const lines = [];
    lines.push('# Asia-1H — Wave Summary');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('## Collision-watch list (user-specified, Asia-1H kickoff)');
    lines.push('');
    ...
    lines.push('| --- | --- |');
    lines.push('| Wave summary (this file) | `reports/geodata-asia-1h-summary.md` |');
    lines.push('| Arabic-quality detail   | `reports/geodata-asia-1h-arabic-quality-report.md` |');
    for (const cc of CCS) {
