...
// ─────────────────────────────────────────────────────────────────────────
// CURATED-GEODATA-ASIA-1I — Strategy E report generator
// (AZ + GE + AM — Caucasus)
//
// Inputs:
//   - db/places/candidates/{cc}-geonames-candidates.json   (Stage 3 + 3.5)
//   - db/places/candidates/asia-1i-arabic-quality.json     (Stage 3.5 summary)
//   - db/places/curated-places.json
//   - db/places/candidates/{cc}-geonames-raw.json          (for originalName)
//
// Outputs (5 files):
//   - reports/geodata-asia-1i-summary.md
//   - reports/geodata-asia-1i-arabic-quality-report.md
//   - reports/{cc}-geodata-import-report.md (az, ge, am)
//
...
const WAVE_LABEL = 'CURATED-GEODATA-ASIA-1I';
const WAVE_SLUG  = 'asia-1i';
const STRATEGY   = 'E';
const CCS        = ['az','ge','am'];

// Watch list: user-flagged collision candidates (Caucasus capitals + secondaries)
const WATCH_LIST = [
    'baku','ganja','sumqayit','mingachevir','lankaran','sheki','shirvan','khirdalan',
    'tbilisi','batumi','kutaisi','rustavi','zugdidi','gori','sokhumi',
    'yerevan','gyumri','vanadzor','hrazdan','ararat','armavir','kapan'
];
...
function renderCountryReport(cc, candidates, rawIndex, config) {
    ...
    lines.push('# ' + upper + ' GeoNames Import Report — Asia-1I');
    lines.push('');
...
function renderArQualityReport(perCountryResults) {
    const lines = [];
    lines.push('# Asia-1I — Arabic-Name Quality Report');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('| Within Asia-1I wave (NP/LK/MV/BT/BN/MM/KH/LA/TL same slug) | ' + waveColl + ' |');
...
function renderSummary(perCountryResults) {
    const lines = [];
    lines.push('# Asia-1I — Wave Summary');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('## Collision-watch list (user-specified, Asia-1I kickoff)');
    lines.push('');
    ...
    lines.push('| --- | --- |');
    lines.push('| Wave summary (this file) | `reports/geodata-asia-1i-summary.md` |');
    lines.push('| Arabic-quality detail   | `reports/geodata-asia-1i-arabic-quality-report.md` |');
    for (const cc of CCS) {
