...
// ─────────────────────────────────────────────────────────────────────────
// CURATED-GEODATA-EUROPE-1B — Strategy E report generator (ES + PT)
//
// Inputs:
//   - db/places/candidates/{cc}-geonames-candidates.json   (Stage 3 output,
//                                                          enriched by Stage 3.5)
//   - db/places/candidates/europe-1b-arabic-quality.json   (Stage 3.5 summary)
//   - db/places/curated-places.json
//   - db/places/candidates/{cc}-geonames-raw.json          (for originalName)
//
// Outputs:
//   - reports/geodata-europe-1b-summary.md
//   - reports/geodata-europe-1b-arabic-quality-report.md
//   - reports/{cc}-geodata-import-report.md   (REWRITTEN per Europe-1B
//                                              template with 14 fields)
//
// Does NOT touch curated_places.json. Does NOT apply anything.
// Cloned + parameterized from `_strategy_e_europe_1a_reports.mjs` so the
...
const WAVE_LABEL = 'CURATED-GEODATA-EUROPE-1B';
const WAVE_SLUG  = 'europe-1b';
const STRATEGY   = 'E';
const CCS        = ['es','pt'];

// Cities the user flagged for explicit collision check (Strategy E §6)
const WATCH_LIST = ['granada','cordoba','valencia','cartagena','toledo','barcelona','sevilla','seville','malaga','porto','braga','coimbra'];
...
function renderCountryReport(cc, candidates, rawIndex, config) {
    ...
    lines.push('# ' + upper + ' GeoNames Import Report — Europe-1B');
    lines.push('');
...
function renderArQualityReport(perCountryResults) {
    const lines = [];
    lines.push('# Europe-1B — Arabic-Name Quality Report');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('');
    lines.push('Same as Europe-1A: Iberian peninsula has Arabic-name candidates');
    ...
    lines.push('| Within Europe-1B wave (ES↔PT same slug) | ' + waveColl + ' |');
...
function renderSummary(perCountryResults) {
    const lines = [];
    lines.push('# Europe-1B — Wave Summary');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('| --- | --- |');
    lines.push('| Wave summary (this file) | `reports/geodata-europe-1b-summary.md` |');
    lines.push('| Arabic-quality detail   | `reports/geodata-europe-1b-arabic-quality-report.md` |');
    for (const cc of CCS) {
