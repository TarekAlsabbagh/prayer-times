...
// ─────────────────────────────────────────────────────────────────────────
// CURATED-GEODATA-EUROPE-3 — Strategy E report generator (ES + PT)
//
// Inputs:
//   - db/places/candidates/{cc}-geonames-candidates.json   (Stage 3 output,
//                                                          enriched by Stage 3.5)
//   - db/places/candidates/europe-3-arabic-quality.json   (Stage 3.5 summary)
//   - db/places/curated-places.json
//   - db/places/candidates/{cc}-geonames-raw.json          (for originalName)
//
// Outputs:
//   - reports/geodata-europe-3-summary.md
//   - reports/geodata-europe-3-arabic-quality-report.md
//   - reports/{cc}-geodata-import-report.md   (REWRITTEN per Europe-3
...
const WAVE_LABEL = 'CURATED-GEODATA-EUROPE-3';
const WAVE_SLUG  = 'europe-3';
const STRATEGY   = 'E';
...
const WATCH_LIST = ['pristina','warsaw','krakow','prague','budapest','bucharest','sofia','athens','zagreb','belgrade','sarajevo','podgorica','skopje','tirana','tallinn','riga','vilnius','valletta','nicosia','wroclaw','gdansk','poznan','varna','plovdiv','thessaloniki','split','rijeka','novi-sad','cluj-napoca','timisoara','kaunas','liepaja'];
...
function renderCountryReport(cc, candidates, rawIndex, config) {
    ...
    lines.push('# ' + upper + ' GeoNames Import Report — Europe-3');
    lines.push('');
...
function renderArQualityReport(perCountryResults) {
    const lines = [];
    lines.push('# Europe-3 — Arabic-Name Quality Report');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('');
    lines.push('Same as Europe-1A: Eastern Europe + Balkans + Baltics + Mediterranean islands have Arabic-name candidates');
    ...
    lines.push('| Within Europe-3 wave (ES↔PT same slug) | ' + waveColl + ' |');
...
function renderSummary(perCountryResults) {
    const lines = [];
    lines.push('# Europe-3 — Wave Summary');
    lines.push('');
    lines.push('**Wave**: `' + WAVE_LABEL + '`');
    ...
    lines.push('| --- | --- |');
    lines.push('| Wave summary (this file) | `reports/geodata-europe-3-summary.md` |');
    lines.push('| Arabic-quality detail   | `reports/geodata-europe-3-arabic-quality-report.md` |');
    for (const cc of CCS) {
